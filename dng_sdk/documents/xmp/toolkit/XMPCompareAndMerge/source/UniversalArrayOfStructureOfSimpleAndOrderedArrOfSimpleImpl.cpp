...
namespace AdobeXMPCompareAndMerge_Int {
	...
	static bool verifyNode ( spcINode node )
	{
		...
		if ( arrayForm != IArrayNode::kAFOrdered && arrayForm != IArrayNode::kAFUnordered && arrayForm != IArrayNode::kAFAlternative )
			return false;
		if ( !AutoMergeUtilities::verifyChildTypeArrOfStructOfSimpleAndOrArr ( arrayNode ) )
			return false;
		return true;
	}
			...
			else
			{
				...
				else
				{
					...
					if (findNode(baseArray, clonedChild))
					{
						...
						while ( it ) {
							spcINode field = it->GetNode ( );
							spINode latestField = clonedChild->GetNode ( field->GetNameSpace ( )->c_str ( ) , AdobeXMPCommon::npos , field->GetName ( )->c_str ( ) , AdobeXMPCommon::npos );
							spcINode userField = userNodeInSet->GetNode ( field->GetNameSpace ( )->c_str ( ) , AdobeXMPCommon::npos , field->GetName ( )->c_str ( ) , AdobeXMPCommon::npos );
							if ( field->ConvertToSimpleNode ( ) )
							{
								AutoMergeUtilities::resolveQualifier ( field , latestField , userField , latestField );
							}
							else   //We know that only Simple and Ordered Array of Simple node are present
							{
								//First look for ordering:
								if ( !AutoMergeUtilities::compareOrderedArrayOfSimple ( latestField->ConvertToArrayNode ( ) , userField->ConvertToArrayNode ( ) , false ) )
								{
									if ( AutoMergeUtilities::compareOrderedArrayOfSimple ( field->ConvertToArrayNode ( ) , latestField->ConvertToArrayNode ( ) , false ) )
									{
										clonedChild->ReplaceNode ( userField->Clone ( )->ConvertToArrayNode ( ) );
										// To-Do qual on array node. As for now qualifier is not handled in this case
									}
								}
								else
								{
									if ( AutoMergeUtilities::compareOrderedArrayOfSimple ( field->ConvertToArrayNode ( ) , latestField->ConvertToArrayNode ( ) , false ) )
									{
										for ( sizet j = 1; j <= field->ConvertToArrayNode ( )->ChildCount ( ); j++ )
										{
											spcISimpleNode fieldSimple = field->ConvertToArrayNode ( )->GetSimpleNodeAtIndex ( j );
											spISimpleNode latestFieldSimple = latestField->ConvertToArrayNode ( )->GetSimpleNodeAtIndex ( j );
											spcISimpleNode userFieldSimple = userField->ConvertToArrayNode ( )->GetSimpleNodeAtIndex ( j );
											AutoMergeUtilities::resolveQualifier ( fieldSimple , latestFieldSimple , userFieldSimple , latestFieldSimple );
										}
									}

									else
									{
										for ( sizet k = 1; k <= latestField->ConvertToArrayNode ( )->ChildCount ( ); k++ )
										{
											spISimpleNode latestFieldSimple = latestField->ConvertToArrayNode ( )->GetSimpleNodeAtIndex ( k );
											spcISimpleNode userFieldSimple = userField->ConvertToArrayNode ( )->GetSimpleNodeAtIndex ( k );
											spcISimpleNode fieldSimple;
											for ( sizet j = 1; j <= field->ConvertToArrayNode ( )->ChildCount ( ); j++ )
											{
												fieldSimple = field->ConvertToArrayNode ( )->GetSimpleNodeAtIndex ( k );
												if ( fieldSimple->GetValue ( )->compare ( latestFieldSimple->GetValue ( ) ) )  //one value will definitely be present
													break;
											}
											AutoMergeUtilities::resolveQualifier ( fieldSimple , latestFieldSimple , userFieldSimple , latestFieldSimple );
										}
	...
	bool APICALL UniversalArrayOfStructureOfSimpleAndOrderedArrOfSimpleImpl::InterestedInResolvingConflict ( const spcIUTF8String & nodePath , INode::eNodeType nodeType , IThreeWayUnResolvedConflict::eConflictReason reason , bool alreadyResolved )
	{
		if ( alreadyResolved )
			return false;
	...
	IThreeWayUnResolvedConflict::eConflictReason APICALL UniversalArrayOfStructureOfSimpleAndOrderedArrOfSimpleImpl::ResolveConflict ( IThreeWayUnResolvedConflict::eConflictReason conflictReason ,
