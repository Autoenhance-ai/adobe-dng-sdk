...
public:
    ...
    /// \brief GetMemProcs returns the internal memory allocation routines used by XMP. These are
    /// either standard C malloc and free, or those passed to \c Initialize. This is not intended
    /// as a general client utility. It is a utility for DLLs layered on top of XMP, so they can
    /// use the same client provided memory routines, if any.
    ...
    /// \brief \c RegisterAssertNotify registers a callback function to provide user notification
    /// of debugging assertion failures. See the \c XMP_AssertNotifyProc declaration in
    /// \c XMP_Const.h for more information.

    static void
    RegisterAssertNotify ( XMP_AssertNotifyProc notifyProc,
                           void *               refCon );

    /// \brief \c UnregisterAssertNotify unregisters a notification function if it is the active one.
    /// Passing a null pointer unregisters any active notification function.

    static void
    UnregisterAssertNotify ( XMP_AssertNotifyProc notifyProc );

    /// \brief \c SendAssertNotify sends an assert notification with the given message. This is not
    /// intended as a general client utility. It is a utility for DLLs layered on top of XMP,
    /// allowing them to support assert notifications without forcing the client app to separately
    /// register the callback.

    static void
    SendAssertNotify ( XMP_StringPtr message );
    ...
    /// \brief \c DumpPropertyTraits dumps the list of registered property traits.
    ///
    ...
    ///
    /// \param propName The name of the property. May be a general path expression, must not be null
    /// or the empty string.
    ///
    ...
    ///
    /// \li \c kXMP_PropIsStale - This is a derived property and the document content is known to
    /// have changed since the property value was set.
