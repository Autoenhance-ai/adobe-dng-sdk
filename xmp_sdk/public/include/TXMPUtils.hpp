...
public:
    ...
    /// external property, or if the \c kXMPUI_DoAllProperties option is passed. It does not matter
    /// whether the named property is an actual property or an alias.
    ///
    /// \li Non-empty \c schemaNS and empty \c propName - The all external properties in the named
    /// schema are removed. Internal properties are also removed if the \c kXMPUI_DoAllProperties
    /// option is passed. In addition, aliases from the named schema will be removed if the \c
    /// kXMPUI_IncludeAliases option is passed.
    ///
    /// \li Empty \c schemaNS and empty \c propName - All external properties in all schema are
    /// removed. Internal properties are also removed if the \c kXMPUI_DoAllProperties option is
    ...
    /// \li \c kXMPUI_DoAllProperties - Do internal properties in addition to external properties.
    /// \li \c kXMPUI_IncludeAliases - Include aliases in the "named schema" case above.
