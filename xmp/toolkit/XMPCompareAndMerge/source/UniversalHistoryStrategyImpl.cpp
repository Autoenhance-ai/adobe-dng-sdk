...
namespace AdobeXMPCompareAndMerge_Int {
	...
	static spINode MergeEntries ( const spcINode & source1 , const spcINode & source2 ) {
		...
		while ( indexInArray1 < sizeOfArray1 && indexInArray2 < sizeOfArray2 ) {
			...
			if ( useBoth || !useNodeInArray2 ) {
				while ( indexInArray1 < sizeOfArray1 && !whenNodeInArray1 ) //if in = size, then think
				{
					...
					whenNodeInArray1 = nodeInArray1->GetSimpleNode ( kXMP_NS_XMP_ResourceEvent , AdobeXMPCommon::npos , "when" , AdobeXMPCommon::npos );
				...
				if ( whenNodeInArray1 ) {
					try {
						SXMPUtils::ConvertToDate ( whenNodeInArray1->GetValue ( )->c_str ( ) , &dateInNode1 ); // catch and think of exception
					}
					catch ( ... )
					{
			...
			if ( useBoth || useNodeInArray2 ) {
				while ( indexInArray2 < sizeOfArray2 && !whenNodeInArray2 ) //if in = size, then think
				{
					...
					whenNodeInArray2 = nodeInArray2->GetSimpleNode ( kXMP_NS_XMP_ResourceEvent , AdobeXMPCommon::npos , "when" , AdobeXMPCommon::npos );
			...
			for ( ; it1 != nodesInArray1.end ( ) && it2 != nodesInArray2.end ( );) {
				if ( AutoMergeUtilities::compareStructs ( *it1 , *it2 , false ) ) {
		...
		{ // we have exhausted all the nodes in array1
			if ( !nodesInArray2.empty ( ) )
			{
				...
				bool valid = false;
				XMP_DateTime date;
				spcISimpleNode dateNode = (nodesInArray2.back ( ))->GetSimpleNode ( kXMP_NS_XMP_ResourceEvent , AdobeXMPCommon::npos , "when" , AdobeXMPCommon::npos );
				if ( dateNode )
				{
					try {
			...
			if ( !nodesInArray1.empty ( ) )
			{
				...
				bool valid = false;
				XMP_DateTime date;
				spcISimpleNode dateNode = (nodesInArray1.back ( ))->GetSimpleNode ( kXMP_NS_XMP_ResourceEvent , AdobeXMPCommon::npos , "when" , AdobeXMPCommon::npos );
				if ( dateNode )
				{
					try {
						SXMPUtils::ConvertToDate ( dateNode->GetValue ( )->c_str ( ) , &date );
	...
	bool APICALL UniversalHistoryStrategyImpl::InterestedInResolvingConflict ( const spcIUTF8String & nodePath , INode::eNodeType nodeType ,
	...
	IThreeWayUnResolvedConflict::eConflictReason APICALL UniversalHistoryStrategyImpl::ResolveConflict (
		IThreeWayUnResolvedConflict::eConflictReason conflictReason ,
		...
		spcINode & resolvedNode , IThreeWayResolvedConflict::eConflictResolutionReason & conflictResolutionReason , IThreeWayResolvedConflict::eResolvedSource & resolvedSource )
	{
		...
		case IThreeWayUnResolvedConflict::kCRQualifiersMismatch:
			if ( !verifyNode ( latestVersionNode ) || !verifyNode ( userVersionNode ) )
				NOTIFY_ERROR ( IError_base::kEDDataModel , kDMECDifferentNodeTypePresent ,
					"History must be of type Unordered Array of ResourceEvent" , IError_base::kESOperationFatal , false , false );
		...
		case IThreeWayUnResolvedConflict::kCRDeletedInLatestEditedInUser:
			if ( !verifyNode ( userVersionNode ) )
				NOTIFY_ERROR ( IError_base::kEDDataModel , kDMECDifferentNodeTypePresent ,
					"Versions must be of type Unordered Array of ResourceEvent" , IError_base::kESOperationFatal , false , false );
			if ( useBase ) //merge or latest ??
			{
