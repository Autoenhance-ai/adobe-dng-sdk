...
namespace AdobeXMPCompareAndMerge_Int {
	...
	static spcIStructureNode findNode ( std::vector<spcIStructureNode> nodes, spcIStructureNode node ) {
		for ( std::vector<spcIStructureNode>::iterator it = nodes.begin (); it != nodes.end (); it++ ) {
			if ( AutoMergeUtilities::compareStructOfSimpleAndStruct ( *it, node, false ) )
				return *it;
		}
				...
				else
				{
					if ( findNode ( baseArray, clonedChild ) )
					{
						...
						while ( it ) {
							...
							else   //We know that only Simple and Structure of Simple node are present
							{
								spcINodeIterator iter = field->ConvertToStructureNode ( )->Iterator ( );
								while ( iter ) {
									spcISimpleNode fieldSimple = iter->GetSimpleNode ( );
									spISimpleNode latestFieldSimple = latestField->ConvertToStructureNode ( )->GetSimpleNode ( fieldSimple->GetNameSpace ( )->c_str ( ) , AdobeXMPCommon::npos , fieldSimple->GetName ( )->c_str ( ) , AdobeXMPCommon::npos );
									spcISimpleNode userFieldSimple = userField->ConvertToStructureNode ( )->GetSimpleNode ( fieldSimple->GetNameSpace ( )->c_str ( ) , AdobeXMPCommon::npos , fieldSimple->GetName ( )->c_str ( ) , AdobeXMPCommon::npos );
	...
	bool APICALL UniversalArrayOfStructureOfSimpleAndStructOfSimpleImpl::InterestedInResolvingConflict ( const spcIUTF8String & nodePath , INode::eNodeType nodeType , IThreeWayUnResolvedConflict::eConflictReason reason , bool alreadyResolved )
	{
		if ( alreadyResolved )
			return false;
	...
	IThreeWayUnResolvedConflict::eConflictReason APICALL UniversalArrayOfStructureOfSimpleAndStructOfSimpleImpl::ResolveConflict ( IThreeWayUnResolvedConflict::eConflictReason conflictReason ,
